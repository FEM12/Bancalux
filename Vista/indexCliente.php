...
$consulta2 = "SELECT nombre, apellido FROM cliente WHERE idCliente = :idCliente";
$stmt2 = $db -> prepare($consulta2);
$stmt2 -> execute([':idCliente' => $usuario]);
...
<body class="bg-slate-900 bg-fixed bg-cover min-h-screen flex flex-col items-center">
  ...
  <nav class="bg-slate-950 bg-opacity-0 w-full p-4">
    <ul class="flex justify-between items-center text-zinc-50 text-lg">
      ...
      <li>
        <a href="../Controlador/cerrarSesionCliente.php" class="text-blue-500">Cerrar Sesión</a>
      </li>
    </ul>
  </nav>
  ...
  <main class="w-full max-w-3xl mx-auto p-8">
    <h2 class="text-3xl font-bold text-center text-white mb-6">¡Bienvenido/a!
      <br>
      <?php foreach ($resultados2 as $fila) {
    ...
    <?php
      if (isset($_GET['opcion'])) {
        echo '<div class="mt-6 text-center text-red-500">Error: Has alcanzado el limite máximo de cuentas</div>';
      }

      if (isset($_GET['sucses'])) {
        echo '<div class="mt-6 text-center text-green-500">Éxito: La cuenta se creo correctamente</div>';
      }
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mt-10">
      ...
      <?php foreach ($resultados as $fila): ?>
        <div class="bg-black/30 p-10 rounded-lg shadow-lg text-center transition-transform duration-300 hover:scale-105">
          <h1 class="text-lg text-white font-bold"><?php echo $fila['numerocuenta']; ?></h1>
    ...
    <h2 class="text-3xl font-bold text-center text-white mb-6 mt-5">Más opciones</h2>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
      ...
      <div class="bg-black/30 backdrop-blur-lg p-6 rounded-lg shadow-lg text-center transition-transform duration-300 hover:scale-105">
        <img src="Recursos/Imagenes/deposito.png" alt="Registrar Clientes" class="w-16 h-16 mx-auto mb-4">
        <h1 class="text-lg font-bold text-white">Crear Cuentas</h1>
        <p class="text-zinc-50">Crea como maximo 3 cuentas de ahorro personales.</p>
        <a href="crearCuentaBancaria.php?id=<?php echo $usuario ?>" class="mt-4 inline-block bg-blue-500 text-white py-2 px-4 rounded-lg shadow hover:bg-blue-600">
          Ingresar
        </a>
      </div>
      ...
      <!-- Card 4 - Préstamos -->
      <div class="bg-black/30 backdrop-blur-lg p-6 rounded-lg shadow-lg text-center transition-transform duration-300 hover:scale-105">
        <img src="Recursos/Imagenes/registro.png" alt="Prestamos" class="w-16 h-16 mx-auto mb-4">
        <h1 class="text-lg font-bold text-white">Movimientos</h1>
        <p class="text-zinc-50">Verifica y administra los movimientos de tus cuentas.</p>
        <a href="historialTransacciones.php?id=<?php echo $usuario ?>" class="mt-4 inline-block bg-blue-500 text-white py-2 px-4 rounded-lg shadow hover:bg-blue-600">
          Ingresar
        </a>
      </div>

      <div class="bg-black/30 backdrop-blur-lg p-6 rounded-lg shadow-lg text-center transition-transform duration-300 hover:scale-105">
        <img src="Recursos/Imagenes/prestamo.png" alt="Prestamos" class="w-16 h-16 mx-auto mb-4">
        <h1 class="text-lg font-bold text-white">Transferencias</h1>
        <p class="text-zinc-50">Envia direro a tus cuentas personales.</p>
        <a href="realizarTransferencia.php?id=<?php echo $usuario ?>" class="mt-4 inline-block bg-blue-500 text-white py-2 px-4 rounded-lg shadow hover:bg-blue-600">
          Ingresar
        </a>
      </div>
      ...
      <div class="bg-black/30 backdrop-blur-lg p-6 rounded-lg shadow-lg text-center transition-transform duration-300 hover:scale-105">
        ...
        <p class="text-zinc-50">Solicita/Apertura prestamos</p>
        <a href="solicitarPrestamo.php?id=<?php echo $usuario ?>" class="mt-4 inline-block bg-blue-500 text-white py-2 px-4 rounded-lg shadow hover:bg-blue-600">
          Ingresar
        </a>
      </div>
    </div>
