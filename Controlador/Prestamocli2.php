...
$validacion_correcta = false; // Variable booleana para verificar si se cumple alguna de las condiciones de validación

if($sueldo > 0 && $sueldo < 365){

    if($prestamo <= 10000){ $validacion_correcta = true; }
    else{
...
elseif ($sueldo > 0 && $sueldo < 600){
    ...
    if($prestamo <= 25000){ $validacion_correcta = true; }
    else{
...
elseif ($sueldo > 0 && $sueldo < 900){
    ...
    if($prestamo <= 35000){ $validacion_correcta = true; }
    else{
...
elseif ($sueldo > 0 && $sueldo > 1000){
    ...
    if($prestamo <= 50000){ $validacion_correcta = true; }
    else{
...
if($validacion_correcta) { // Se realiza la inserción solo si se cumple alguna de las condiciones de validación
    ...
    try {
        ...
        $stmt->execute();
        header("location:../Vista/index.php?prestamo=prestamo");
